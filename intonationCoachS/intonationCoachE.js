...
const EMBEDDED = paramObj.embedded === '1';	// 区間エディタ等からダイアログ/iframeとして起動されたか
...
// SwiftF0と全く同じフレーム位置（HOP_LENGTH刻み・FRAME_LENGTH窓）でRMSを計算する。
// こうすることで、返り値の配列は pitchData/confData と同じインデックスでそのまま対応する
// （SwiftF0自体は音量情報を一切出力しないため、別途自前で求める）。
function computeRms(audio16k) {
	const n = Math.max(0, Math.floor((audio16k.length - FRAME_LENGTH) / HOP_LENGTH) + 1);
	const rms = new Float32Array(n);
	for (let i = 0; i < n; i++) {
		const start = i * HOP_LENGTH;
		let sumSq = 0;
		for (let j = 0; j < FRAME_LENGTH; j++) {
			const v = audio16k[start + j] || 0;
			sumSq += v * v;
		}
		rms[i] = Math.sqrt(sumSq / FRAME_LENGTH);
	}
	return rms;
}

// ピッチ点の色を、その点の相対的な音量（0=最小 〜 1=そのデータ内での最大）に応じて
// グラデーションで変化させるためのカラーマップ。モデル(ファイル)音源用とマイク音源用で
// 別の配色を使う。
class ModelColor {
	static STOPS = [
		{ pos: 0.0, r: 51, g: 207, b: 255},
		{ pos: 0.50, r: 153, g: 230, b: 255},
		{ pos: 1.0,  r: 255, g: 255, b: 255},
	];
	getColor(t) {
		t = Math.max(0, Math.min(1, t));
		let idx = 0;
		while (idx < ModelColor.STOPS.length - 2 && ModelColor.STOPS[idx + 1].pos < t) {
			idx++;
		}

		const c1 = ModelColor.STOPS[idx];
		const c2 = ModelColor.STOPS[idx + 1];
...
const modelColorMap = new ModelColor();
const micColorMap = new MicColor();
...
// --- 埋め込みモード（postMessageで区間音声を受け取る）関連の状態変数 ---
let pendingSegment = null;			// モデル初期化が終わる前に届いた分析リクエストを保持しておく
...
async function initModel() {
	try {
		...
		if (EMBEDDED) {
			// 親ウィンドウ（区間エディタ等）へ準備完了を知らせ、待たされていた
			// 分析リクエストがあればここで処理する。
			...
			if (pendingSegment) {
				...
				pendingSegment = null;
				handleIncomingSegment(seg.sampleRate, seg.samples);
			}
		}
	} catch (err) {
		console.error(err);
...
async function analyzeFullBuffer(buffer) {
	...
	await analyzeMonoPCM(mono, nativeRate);
}

// モノラルPCM（任意のサンプリングレート）を16kHzへリサンプルしてSwiftF0で分析する。
// 呼び出し側が既に16kHzで渡してきた場合（例: 区間エディタ側で変換済みの音声）は
// OfflineAudioContextでのリサンプルを丸ごとスキップする。
// 戻り値: 分析に成功して fileAnalysisData を更新できたら true。
async function analyzeMonoPCM(monoSamples, nativeRate) {
	try {
		let audio16k;
		if (nativeRate === TARGET_SR) {
			audio16k = monoSamples;
		} else {
			const outLen = Math.max(1, Math.ceil(monoSamples.length * TARGET_SR / nativeRate));
			const offlineCtx = new OfflineAudioContext(1, outLen, TARGET_SR);
			const src = offlineCtx.createBufferSource();
			const buf = offlineCtx.createBuffer(1, monoSamples.length, nativeRate);
			buf.copyToChannel(monoSamples, 0);
			src.buffer = buf;
			src.connect(offlineCtx.destination);
			src.start(0);
			const rendered = await offlineCtx.startRendering();
			audio16k = rendered.getChannelData(0);
		}
		monoSamples = null; // 用済みのため解放
		...
		const rmsData = computeRms(audio16k);
		let rmsMax = 0;
		for (let i = 0; i < rmsData.length; i++) {
			if (rmsData[i] > rmsMax) rmsMax = rmsData[i];
		}
		...
		fileAnalysisData = {
			...
			confData: confData,
			rmsData: rmsData,
			rmsMax: rmsMax || 1e-9, // 無音等でmaxが0の場合の0除算を避ける
...
/* ********************************************************************************
 * 埋め込みモード: postMessage経由で区間音声を受け取って分析する
 * ---------------------------------------------------------------------------
 * 区間エディタ側（親ウィンドウ / iframeの起動元）から、以下の形式のメッセージを
 * 受け取る想定:
 *   { type: 'analyze-segment', sampleRate: <Number>, samples: <ArrayBuffer> }
...
async function runSegmentAnalysis(sampleRate, floatSamples) {
	...
	// 再生用に、受け取ったPCMをそのままWAV化してAudio要素にセットする
	// （マイク録音の再生と同じ pcmToWavBlob() をそのまま流用できる）。
	const wavBlob = pcmToWavBlob(floatSamples, sampleRate);
	if (fileObjectURL) URL.revokeObjectURL(fileObjectURL);
	fileObjectURL = URL.createObjectURL(wavBlob);
	audioEl.src = fileObjectURL;
	audioEl.playbackRate = parseFloat(UI.fileSpeedSlider.value);

	UI.statusDiv.textContent = LITERALS.get('statusAnalysingSegment');
	const ok = await analyzeMonoPCM(floatSamples, sampleRate);

	layoutAndRender();
	if (ok && fileAnalysisData) {
		UI.playPauseBtn.disabled = false;
		UI.fileSpeedSlider.disabled = false;
		setScrollLeftAndRender(0);
...
// 親（区間エディタ）のダイアログが閉じられた際に届く。バックグラウンドで音が
// 鳴り続けたりrequestAnimationFrameループが回り続けたりしないよう再生を止め、
// 再生用に作ったWAVのobject URLもその場で解放する（次の分析まで持ち越さない）。
function suspendForEmbed() {
	stopFilePlayback();
	stopMicPlayback();
	if (fileObjectURL) {
...
window.addEventListener('message', (e) => {
	...
	if (data.type === 'ic-suspend') {
		suspendForEmbed();
		return;
	}
	if (data.type !== 'analyze-segment') return;
	const floatSamples = data.samples instanceof ArrayBuffer ? new Float32Array(data.samples) : null;
	if (!floatSamples || !data.sampleRate) return;
	handleIncomingSegment(data.sampleRate, floatSamples);
});
...
function render() {
	...
	function plotSeries(analysisData, colorMap, minConf, timeShift) {
		if (!analysisData) return;
		const { pitchData, confData, rmsData, rmsMax, duration } = analysisData;
		...
		for (let i = iStart; i <= iEnd; i++) {
			...
			// 音量(RMS)をそのデータ内での相対値[0,1]に正規化し、色と点の大きさ(1〜3)に反映する。
			const loud = rmsData ? (rmsData[i] || 0) : 0;
			const t = rmsMax > 0 ? Math.min(1, loud / rmsMax) : 0;
			const radius = 1 + 2 * t; // 1(小さい/静か) 〜 3(大きい/大きな声)

			pitchCtx.fillStyle = colorMap.getColor(t);
			pitchCtx.beginPath();
			pitchCtx.arc(x, y, radius, 0, Math.PI * 2);
			pitchCtx.fill();
		}
	}

	plotSeries(fileAnalysisData, modelColorMap, parseFloat(UI.confThresholdInput.value), 0);						// ファイル分析
	plotSeries(micAnalysisData, micColorMap, parseFloat(UI.confThresholdMicInput.value), micTimeOffset);	// マイク録音
